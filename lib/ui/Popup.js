...
Popup.prototype = {
  ...
  onMessage: function(id, msg, sender) {
    ...
    if (msg.type == 'autocomplete-url-clicked') {
      ...
      if (msg.data.resultType == 'url') {
        ...
        gURLBar.inputField.value = msg.data.result;
        gBrowser.loadURI(msg.data.result);
      } else {
      ...
      if (msg.data.resultType == 'url') {
        ...
        gURLBar.value = msg.data.result;
        url = engine.getSubmission(msg.data.result).uri.spec;
        gURLBar._search = { term: msg.data.result, url: url };
      }
    }
  },
  onPopupShowing: function() {
    if (!this.port) { return; }
    this.port.send({ type: 'popupopen' }, this.browser);
  },
  onPopupHiding: function() {
    if (!this.port) { return; }
    this.port.send({ type: 'popupclose' }, this.browser);
  },
  ...
  _appendCurrentResult: function() {
    ...
    this.port.send({ type: 'autocomplete-search-results', data: autocompleteResults }, this.browser);
    this.port.send({ type: 'suggested-search-results', data: searchSuggestions}, this.browser);
  },
