...
Popup.prototype = {
  ...
  render:  function(win) {
    ...
    this.popup.addEventListener('mousedown', function() { console.log('popup mousedown') });
    this.popup.addEventListener('mouseup', function() { console.log('popup mouseup') });
    this.popup.addEventListener('click', function() { console.log('popup click') });
    this.popup.addEventListener('mousedown', this.onMouseDown.bind(this));
    this.popup.addEventListener('blur', function() { console.log('popup blur') });
    this.popup.addEventListener('focus', function() { console.log('popup focus') });
  ...
  setBrowser: function(browserEl) {
    ...
    browserEl.addEventListener('blur', function() { console.log('iframe blur') });
    browserEl.addEventListener('focus', function() { console.log('iframe focus') });
    browserEl.addEventListener('mousedown', function() { console.log('iframe mousedown') });
    browserEl.addEventListener('mouseup', function() { console.log('iframe mouseup') });
    browserEl.addEventListener('click', function() { console.log('iframe click') });
  },
  ...
  onMouseDown: function() {
    console.log('panel received mousedown');
  },
  ...
  onPopupHiding: function(evt) {
    ...
    // XXX this is a hack: because the popup loses track of clicks
    //     if a mousemove occurs during the click, the focus is not
    //     consistently returned to the urlbar, which forces the user
    //     to click the urlbar 2 times to get focus into it. So, let's
    //     fire a dummy click on the popup, then, focus the urlbar. fml.
      var params = {
        'view': window,
        'bubbles': true,
        'cancelable': true
      };
      var fakeMouseDown = new MouseEvent('mousedown', params);
      var fakeMouseUp = new MouseEvent('mouseup', params);
      var fakeClick = new MouseEvent('click', params);
      if (window.US.popup && window.US.popup.popup) {
        window.US.popup.popup.dispatchEvent(fakeMouseDown);
        window.US.popup.popup.dispatchEvent(fakeMouseUp);
        window.US.popup.popup.dispatchEvent(fakeMouseClick);
      }
